...
from rq_queue import metrics_queue, redis_connection
...
def process_metrics(data: Any) -> None:
    """Log metrics jobs without applying ML processing yet."""
    logger.info("received metrics job: %s", data)
...
def main() -> None:
    ...
    worker = Worker([metrics_queue], connection=redis_connection, serializer=JSONSerializer)
